...
def create_skipped_status_check(
    *, project: Project, repo_name: str, sha: str, check_type: StatusCheckType
) -> str:
    ...
    client, repository = get_status_check_client_for_repo(project, repo_name)
    if not client or not repository:
        raise SkipStatusCheckError(
            f"No active GitHub integration found for repository '{repo_name}'.",
            status_code=400,
            reason="repo_not_integrated",
        )

    provider_slug = (repository.provider or "").removeprefix("integrations:")
    ...
    if not provider:
        raise SkipStatusCheckError(
            f"Status checks are not supported for provider '{provider_slug}'.",
            status_code=400,
    ...
    try:
        ...
        # Config errors (e.g. missing GitHub App permissions) aren't retryable —
        # surface a 4xx with the provider's actionable message, prefixed so our
        # response shape doesn't silently inherit provider wording verbatim.
        raise SkipStatusCheckError(
            f"Could not post status check: {e}",
            status_code=400,
            reason="config_error",
        ) from e
