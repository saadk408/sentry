...
import {TRIGGER_META} from './triggerBadge';
import type {AttentionReason, AutofixOutcome, AutofixTrigger} from './types';

// Include every trigger source, not just autofix's own, so the overview shows
// all explorer runs regardless of what started them.
const OVERVIEW_RUNS_QUERY = 'type:explorer';
...
export default function AutofixOverview() {
  ...
  const {issues, isPending, isError, refetch, pageLinks} = useAutofixIssues({
    ...
    // resolved by a merged autofix PR are exactly the wins this page should
    // still show. The has-seer-run filter is always applied by the hook.
    query: '',
    cursor,
  ...
  const mergeDataAvailable = rowsWithAttention.some(
    ({row}) => row.prMerged !== undefined
  );

  const stats = {reviewPr: 0, awaitingInput: 0, codeChangesReady: 0, merged: 0};
      ...
      <Flex direction="column" gap="lg" padding="xl">
        ...
        <Container width={{md: '100%', lg: '85%'}}>
          ...
          >
            ...
            />
            {/* Live once the runs API returns pullRequests; honest '—' until
                that backend change is deployed */}
            <StatCard
              Icon={IconMerge}
              iconVariant="success"
              label={t('Merged PRs')}
              value={mergeDataAvailable ? stats.merged : '—'}
              isActive={quickFilter === 'merged'}
              onClick={mergeDataAvailable ? () => toggleQuickFilter('merged') : undefined}
              extra={
                mergeDataAvailable ? undefined : (
                  <InfoTip
                    size="xs"
                    title={t(
                      "PR merge status isn't available from the API yet — coming with the runs endpoint's pullRequests field."
                    )}
                  />
                )
              }
          ...
          {/* No pagination while loading/errored — arrows for content that
              isn't there yet just add chrome to a spinner */}
