...
class OrganizationSeerRunsEndpointTest(APITestCase):
    ...
    def test_pull_requests_serialized_with_state(self) -> None:
        ...
        prs_by_key = {pr["key"]: pr for pr in by_id[str(run.uuid)]["pullRequests"]}
        assert prs_by_key["123"]["state"] == "merged"
        assert prs_by_key["123"]["mergedAt"] == merged_at.isoformat()
        # No webhook has reported a state for the second PR.
        assert prs_by_key["124"]["state"] is None
